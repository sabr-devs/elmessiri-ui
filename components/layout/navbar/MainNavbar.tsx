    ...
    <>
      ...
      >
        <nav className="container relative flex items-center">
          ...
          <ul
            className="nav-list mx-auto hidden bg-white p-2.5 shadow-nav dark:bg-dark-200 lg:flex [&>*:not(:last-child)]:me-1"
            dir="rtl"
          >
            {MenuData.menuContent.map((menuItem: MenuItem) => (
              <li
              ...
              >
                {menuItem.path ? (
                  <Link
                    href={menuItem.path}
                    className={cn(
                      "flex items-center border border-transparent px-5 py-[5px] font-Inter text-base font-medium capitalize leading-8 text-paragraph transition-colors duration-500 hover:border-borderColor hover:bg-white hover:duration-500 dark:text-white dark:hover:border-borderColor/10 dark:hover:bg-dark-200 lg:px-4 xl:px-5"
                    )}
                  >
                    {menuItem.title}
                  </Link>
                  ...
                  <>
                    ...
                    >
                      ...
                      <FontAwesomeIcon
                        icon={faAngleDown}
                        className="mr-1 mt-1 text-paragraph duration-500 group-hover:rotate-180 dark:text-white"
                      />
                    </Link>
                    <ul className="absolute right-0 top-12 z-10 min-w-[250px] origin-top scale-y-0  bg-white p-5 opacity-0 duration-500 group-hover:scale-y-100 group-hover:opacity-100 dark:bg-dark-200 [&>*:not(:first-child)]:mt-2.5 [&>*:not(:last-child)]:border-b [&>*:not(:last-child)]:border-dashed [&>*:not(:last-child)]:border-borderColor dark:[&>*:not(:last-child)]:border-borderColor-dark">
                      {menuItem?.submenu &&
          ...
          <ul className="ml-auto flex items-center [&>*:not(:last-child)]:me-2.5">
            <li>
              <button
                ...
                className="rounded-full  p-2.5 dark:bg-dark-200"
                id="open-btn"
              >
                <svg
                  width="20"
          ...
          <div
            className={`mobile-menu max-lg:overflow-y-auto ${
              showMobileMenu ? "open" : ""
            }`}
          >
            <button
                ...
                >
                  {menuItem.path ? (
                    <Link
                      href={menuItem.path}
                      className={cn(
                        "flex items-center  border border-transparent px-5 py-[5px] font-Inter text-base font-medium leading-8 text-paragraph transition-colors duration-500 hover:border-borderColor hover:bg-white hover:duration-500 dark:text-white dark:hover:border-borderColor/10 dark:hover:bg-dark-200 lg:px-4 xl:px-5"
                      )}
                      onClick={() => setShowMobileMenu(!showMobileMenu)}
                    >
                      {menuItem.title}
                    ...
                    <>
                      <Link
                        href="#"
                        className="flex items-center  border border-transparent px-5 py-[5px] font-Inter text-base font-medium leading-8 text-paragraph transition-colors duration-500 hover:border-borderColor hover:bg-white hover:duration-500 dark:text-white dark:hover:border-borderColor/10 dark:hover:bg-dark-200 lg:px-4 xl:px-5"
                      >
                        {menuItem.title}
                        <FontAwesomeIcon
                          icon={faAngleDown}
